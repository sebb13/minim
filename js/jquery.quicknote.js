...
;(function($, window, document, undefined) {
	...
	QuickNote.prototype = {
		...
		appendElem: function() {
			...
			var showHide = '<div id="qn_sh"><span>notes</span></div>';
			var divNotes = '<div id="notes"></div>';
			var notesInp = '<p><input type="text" name="qn_input" maxlength="500" placeholder="notes..."></p>';
			...
			if (this.config.storage === true) {
				...
				if (ls) {
					...
					$.each(ls, function(index, obj) {
						...
						$('<span class="qn_close"></span>').prependTo('#' + ls[index].id);
		...
		completeNote: function() {
			...
			this.$el.on('keypress', '#notes input', function(e) {
				...
				if (e.which === 13 || e.keyCode === 13) {
					...
					if (notesInpVal) {
						...
						$('<span class="qn_close"></span>').prependTo('#qn_' + uniqid);
			...
			// CLICK TO qn_close NOTES
			this.$el.on('click', '#notes .qn_close', function() {
				$(this).each(function() {
