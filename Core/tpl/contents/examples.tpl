<!-- container -->
<div class="container">
	<ol class="breadcrumb">
		<li><a href="home.html">Accueil</a></li>
		<li class="active">Exemples</li>
	</ol>
	<div class="row">
		<!-- Article main content -->
		<div class="col-sm-12 maincontent">
			<header class="page-header">
				<h1 class="page-title">{__TITLE__} <i class="fa fa-external-link"></i></h1>
				<h3>{__INTRO__}</h3>
			</header>
		</div>
		<!-- /Article -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">sebastien-boulard.webearthquake.com</h4>
			<p>Il s'agit ici du site pro de Sébastien Boulard, développeur freelance.</p>
			<p>Modules utilisés : Contact, Download et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://sebastien-boulard.webearthquake.com" target="_blank">https://sebastien-boulard.webearthquake.com</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">tilala.net</h4>
			<p>Une webradio, bel exemple de l'utilisation de l'AJAX.</p>
			<p>Modules utilisés : Contact, Download et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://tilala.net" target="_blank">https://tilala.net</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">webearthquake.com</h4>
			<p>Webearthquake est un collectif de freelances...</p>
			<p>Modules utilisés : Contact et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://webearthquake.com" target="_blank">https://webearthquake.com</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">lagrainefolle.fr</h4>
			<p>lagrainefolle, un projet de fou !</p>
			<p>Modules utilisés : Contact et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://lagrainefolle.fr" target="_blank">https://lagrainefolle.fr</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">biscuitskhaya.lagrainefolle.fr</h4>
			<p>Des biscuits pour chien !</p>
			<p>Modules utilisés : Contact et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://biscuitskhaya.lagrainefolle.fr" target="_blank">https://biscuitskhaya.lagrainefolle.fr</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">www.streetswing.net</h4>
			<p>Du jazz manouche !</p>
			<p>Modules utilisés : Contact et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://www.streetswing.net" target="_blank">https://www.streetswing.net</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
	<div class="row jumbotron-examples">
		<div class="col-sm-6">
			<h4 style="color: #72a3ff;">www.horslimites13.fr</h4>
			<p>Le club de motards de Gardanne (13) !</p>
			<p>Modules utilisés : Contact, Media, Galleries et un module propre au site.</p>
		<!-- /Article -->
		</div>
		<div class="col-sm-6">
			<h4>&nbsp;</h4>
			<p>
				<i class="fa fa-external-link" style="color: #72a3ff;"></i>&nbsp;
				<a href="https://www.horslimites13.fr" target="_blank">https://www.horslimites13.fr</a>
			</p>
		</div>
		<!-- /Sidebar -->
	</div>
</div>
<!-- /container -->